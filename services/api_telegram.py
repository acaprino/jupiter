    ...
    async def _process_queue(self):
        ...
        while True:
            ...
            try:
                ...
                self.critical(f"Error processing API call in _process_queue", exc_info=e)
            finally:
    ...
    async def _execute_api_call(self, method, agent, *args, **kwargs):
        ...
        while retries < max_retries:
            try:
                ...
                self.error(f"Temporary error. Retrying in 5 seconds...", exc_info=e)
                await asyncio.sleep(5)
            except Exception as e:
                self.critical(f"Unexpected error during API call", exc_info=e)
                raise  # Re-raise the exception to be caught in _process_queue
            retries += 1
        self.error("Exceeded maximum retries for API call.", exc_info=False)
    ...
    async def shutdown(self):
        ...
        if self.worker_task:
            ...
            try:
                ...
                self.critical(f"Error during worker task cancellation", exc_info=e)
        else:
